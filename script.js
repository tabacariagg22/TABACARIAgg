...
SP:14, RJ:16, MG:16, ES:14, PR:18, SC:18, RS:20,
DF:22, GO:22, MT:24, MS:24, BA:24, SE:26, AL:26, PE:26, PB:26, RN:26, CE:28, PI:28, MA:28,
TO:28, PA:30, AM:34, AP:34, RR:36, AC:34, RO:32, default:24
};
...
let state = {user: null, cart: {}, xp: 0, coins: 0, chestOpened: false, bonusPct: 0, soundOn: false};
...
let audioCtx = null;
function beep(freq=660, ms=80, type=‘sine’, vol=.05){
...
function chime(){ beep(880,90,‘triangle’,.05); setTimeout(()=>beep(1320,120,‘triangle’,.04),80); }
function reward(){ beep(660,80,‘square’,.04); setTimeout(()=>beep(990,100,‘square’,.04),90); setTimeout(()=>beep(1320,140,‘square’,.04),200); }
...
setTimeout(()=>$(’#loginForm’).style.display = ‘block’, 300);
}
}, 180 + i*60);
});
}
...
[…e.target.querySelectorAll(‘input,select’)].forEach(el=>{if(!el.value.trim()) el.style.borderColor = ‘var(–danger)’;});
return;
}
...
const c1 = $(’#productsCarousel’); c1.innerHTML=’’;
const c2 = $(’#productsCarousel2’); c2.innerHTML=’’;
...
div.innerHTML = `<div class="img-wrap"> <span class="tag">${p.tag}</span> ${p.tag !== 'novo' ?`<span class="tag ${secondTagClass}" style="top:38px">${secondTagLabel}</span>`:''} <img src="${p.img}" alt="${p.name}" loading="lazy" style="width:100%;height:100%;object-fit:cover;" onerror="this.parentElement.style.background='linear-gradient(135deg,#0a1410,#000)';this.style.display='none'"/> </div> <div class="info"> <h3>${p.name}</h3> <div class="prices"> <span class="old">${fmt(p.old)}</span> <span class="now">${fmt(p.price)}</span> </div> <div class="qty-row"> <div class="qty"> <button data-act="dec" data-id="${p.id}">−</button> <span data-qty="${p.id}">${inCart}</span> <button data-act="inc" data-id="${p.id}">+</button> </div> <button class="add-btn ${inCart? 'added':''}" data-add="${p.id}">${inCart ? 'No carrinho' : 'Adicionar'}</button> </div> </div> `;
...
if(delta > 0){state.xp += 12; state.coins += 5; chime();}
else{beep(300,60,‘sine’,.04);}
saveState();
syncQtyDisplays();
renderCart();
pulseFab();
...
let pct = 0;
for(const t of DISCOUNT_TIERS) if(subtotal >= t.min) pct = t.pct;
return pct + (state.bonusPct || 0);
}
...
let currentTier = -1;
for(let i=0;i<DISCOUNT_TIERS.length;i++) if(t.subtotal >= DISCOUNT_TIERS[i].min) currentTier = i;
...
row.innerHTML = `<img src="${p.img}" alt="" /> <div class="ci-info"> <h4>${p.name}</h4> <div class="ci-price">${fmt(p.price)} × ${qty} = ${fmt(p.price*qty)}</div> <div class="ci-actions"> <div class="qty"> <button data-act="dec" data-id="${p.id}">−</button> <span>${qty}</span> <button data-act="inc" data-id="${p.id}">+</button> </div> <button class="remove" data-rem="${p.id}">Remover</button> </div> </div>`;
...
else if(t.dataset.rem) {delete state.cart[t.dataset.rem]; saveState(); renderCart(); syncQtyDisplays();}
}
...
function checkout(){
const items = Object.entries(state.cart);
if(!items.length){shake($(’#openCart’)); return;}
const t = calcTotals();
const u = state.user;
...
function enc(s){return encodeURIComponent(s).replace(/%0A/g,’%0A’);}
...
function openChest(){
if(state.chestOpened){flashUnlock(‘Você já abriu o baú hoje’); return;}
const bonuses = [2, 3, 5];
